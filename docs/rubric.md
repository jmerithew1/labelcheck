...
- **C9 — MET (as designed: advisory + documented limitation)** Bold prefix — Sonnet parallel judgment, measured 16/17 ([spike-results.md](spike-results.md)); surfaced as advisory with the number; limitation in [approach.md](approach.md); batch **Confirm bold** strip makes the human glance batch-shaped — warning-crop grid, confirm/flag per label, flags escalate to Needs review, decisions export in the CSV `bold_check` column (verified in-browser on the sample batch)
