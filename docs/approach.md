...
- **Bold spot-check strip** (batch) — **now shipped, attention-only**: after a run, the measurement gate resolves every label it can ("bold ✓ measured" in the row summary); if any labels still need a human glance, the strip appears *by itself*, showing only those rows as zoomed warning crops ("6 of 11 verified by measurement — these are the 5 that need your eyes"). Confirm or flag each; flags escalate the row to *Needs review*; when the last one is resolved the strip and its chip disappear entirely. Every decision exports in the report's `bold_check` column (confirmed / flagged / auto_verified / auto_flagged / unconfirmed). The result: a 250-label batch asks the human for exactly the glances the machine couldn't safely make, and nothing else.
