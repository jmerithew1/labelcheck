...
1. **Bold detection is advisory (16/17 measured) and its miss is exactly the evasion case** — an all-caps-but-not-bold prefix reads as "bold" to the model. Red-teaming confirmed such a label would otherwise pass green, so the check is deliberately hedged against failing open: every warning pass carries an explicit "bold is AI-judged — glance at the image" note with the measured number attached, and the batch **Confirm bold** strip makes the glance a one-page task. To be precise about what was and wasn't tried: two approaches were *measured* — the VLM judgment (16/17) and pixel ink-density measurement (killed by a 48-case font/resolution/compression matrix; see decisions.md). A third — **stroke-width/texture modeling** (SWT-style features normalized by glyph height, or a classifier trained on the label generator's unlimited labeled crops) — is the credible next experiment, deliberately deferred at prototype scope, not ruled out. Two things survive any detector: born-digital PDFs can carry font metadata (a deterministic bold check for that slice), and at TTB's recommended submission quality strokes are 1–3 px, so some real submissions physically lack the signal — which is why the advisory + human-confirm wrapper stays regardless of the detector's accuracy.
