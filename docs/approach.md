...
- **Testing & measurement:** Vitest (85 engine tests), Playwright (label rendering, screenshot QA, live-app verification), bespoke harnesses checked into the repo — spike benchmarks, highlight-accuracy gate (≥90% enforced), degraded-fidelity run, batch load run, bold-densitometry matrix.
...
8. **XLSX manifests** are not supported (CSV only) — cut per the pre-approved degradation ladder; the sample CSV makes the format unambiguous.
...
- **Agent ruling on a reviewed row** — **shipped**: the detail panel carries a *Your review* bar — **Reviewed — OK** re-files the row as Matched with a "Reviewed ✓" pill, **Needs correction** keeps it in review as the agent's confirmed finding, and the same bar can change a bold decision made earlier. The ruling outranks every machine state and exports as the report's `agent_review` column (ok / correction / blank), so the human's judgment leaves with the record — the tool's decisions are never the last word.
