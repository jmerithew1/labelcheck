// Multi-signal bold gate — ROUND 3 (final round of the bounded loop).
// Round-2 result: train 56% / validation 36% coverage at 0 confident
// mistakes; misses traced to (a) integer run-length quantization at small
// sizes (bold 5px vs regular 4px = one coarse 1.25 step; semibold
// indistinguishable), (b) OCR dying inside blur/glare warning crops,
// (c) serif/joint pixels polluting the stroke median.
// Round-3 fixes:
//   - ALL measurement happens on the 3x smooth upscale (sub-original-pixel
//     stroke resolution; raw pass removed)
//   - stem-filtered stroke sampling: only pixels on vertical stems
//     (runV >= 1.5 * runH) contribute, falling back to all-ink median when
//     stems are too few
//   - geometric prefix fallback when OCR fails inside the warning crop:
//     first text line located by ink rows; prefix = leading ~28% segment,
//     body = trailing segment of the same line
// Run from samples/tools:  node bold-multisignal-r3.mjs <outDir>
...
const MEASURE_FN = `
  ...
  globalThis.measure = (rows, bg) => {
    ...
    // Stem-filtered widths: vertical stems carry the cleanest weight signal;
    // serifs, joints, and horizontals pollute the median.
    const stems = [], all = [];
    for (let y = 0; y < h; y++)
      for (let x = 0; x < w; x++)
        if (ink[y][x]) {
          all.push(Math.min(runH[y][x], runV[y][x]));
          if (runV[y][x] >= 1.5 * runH[y][x]) stems.push(runH[y][x]);
        }
    const pickFrom = stems.length >= 30 ? stems : all;
    if (pickFrom.length < 20) return null;
    pickFrom.sort((a, b) => a - b);
    return { capH, inkFrac: inkPx / totPx, sw: pickFrom[Math.floor(pickFrom.length / 2)], stems: stems.length >= 30 };
  };
`;
...
async function measureGen(file, mime, boxes) {
  ...
  return await measurePage.evaluate(async ({ b64, mime, boxes, MEASURE_FN }) => {
    ...
    // Round 3: always measure on the 3x upscale — sub-original-pixel stroke
    // resolution is what separates semibold from bold and 4px from 5px.
    const m = pair(3);
    return m ? { ...m, preprocessed: true } : null;
...
async function measureFile(file, mime, wbox, pad = 0.03) {
  ...
  let prefix = words.find((w) => clean(w.text).startsWith('GOVERNMENT'));
  let body = words.find((w) => clean(w.text).startsWith('ACCORDING')) ??
             words.find((w) => clean(w.text).startsWith('CONSUMPTION')) ??
             words.find((w) => clean(w.text).startsWith('BEVERAGES'));
  if (!prefix || !body) {
    // Geometric fallback: locate the first text line by ink rows; the prefix
    // is its leading ~28% segment, the body reference its trailing segment.
    const geo = await measurePage.evaluate(async ({ b64, MEASURE_FN }) => {
      eval(MEASURE_FN);
      const img = new Image();
      ...
      const rows = lumRows(ctx, 0, 0, c.width, c.height);
      const bg = modalBg(rows);
      const w = c.width, h = rows.length;
      const inkRow = rows.map((row) => { let n = 0; for (const v of row) if (Math.abs(v - bg) > 55) n++; return n; });
      let y0 = -1, y1 = -1;
      for (let y = 0; y < h; y++) {
        if (inkRow[y] > w * 0.02) { if (y0 < 0) y0 = y; y1 = y; }
        else if (y0 >= 0 && y - y1 > 3) break;
      }
      if (y0 < 0 || y1 - y0 < 4) return null;
      const rowSlice = rows.slice(y0, y1 + 1);
      let x0 = -1, x1 = -1;
      for (let x = 0; x < w; x++) {
        let any = false;
        for (const row of rowSlice) if (Math.abs(row[x] - bg) > 55) { any = true; break; }
        if (any) { if (x0 < 0) x0 = x; x1 = x; }
      }
      if (x0 < 0 || x1 - x0 < 60) return null;
      const lineW = x1 - x0;
      return {
        prefix: { x0, y0, x1: Math.round(x0 + lineW * 0.28), y1 },
        body: { x0: Math.round(x0 + lineW * 0.34), y0, x1, y1 },
      };
    }, { b64: prep, MEASURE_FN });
    if (!geo) return null;
    prefix = geo.prefix;
...
let best = null;
for (let swHi = 1.05; swHi <= 1.7; swHi += 0.02) {
  for (let swLo = 0.85; swLo <= 1.3; swLo += 0.02) {
...
fs.writeFileSync(path.join(OUT, 'multisignal-r3-results.json'), JSON.stringify({ results, gate: best }, null, 2));
console.log(`\nRaw results in ${OUT}/multisignal-r3-results.json`);
