// CROP-LENS experiment (single bounded extension of the gate loop, same
// zero-confident-mistake rule): the AI stroke-weight judgment previously saw
// the WHOLE label; here it judges the same 3x-upscaled, contrast-stretched
// warning CROP the pixel measurement uses. The gate's confident-bold branch
// requires AI agreement, so a sharper AI signal converts directly into
// coverage. Pixel signals and train/val split are identical to round 2;
// full-label AI results are kept for standalone comparison.
// Run from samples/tools:  node bold-croplens.mjs <outDir>
...
// Full-label AI judgments from prior rounds — comparison baseline only.
...
async function measureGen(file, mime, boxes) {
  ...
  return await measurePage.evaluate(async ({ b64, mime, boxes, MEASURE_FN }) => {
    ...
    // Crop lens: the SAME pre-processed view (3x upscale + contrast stretch)
    // of the whole warning region, for the AI to judge.
    let cropB64 = null;
    if (boxes.warn) {
      const bx = boxes.warn;
      const x0 = Math.max(0, Math.round(bx.x - 4)), y0 = Math.max(0, Math.round(bx.y - 4));
      const w = Math.min(img.naturalWidth - x0, Math.round(bx.w + 8));
      const h = Math.min(img.naturalHeight - y0, Math.round(bx.h + 8));
      if (w > 8 && h > 4) {
        const wc = document.createElement('canvas');
        wc.width = w * 3; wc.height = h * 3;
        const wctx = wc.getContext('2d');
        wctx.imageSmoothingEnabled = true;
        wctx.imageSmoothingQuality = 'high';
        wctx.drawImage(img, x0, y0, w, h, 0, 0, wc.width, wc.height);
        const d = wctx.getImageData(0, 0, wc.width, wc.height);
        ...
        cropB64 = wc.toDataURL('image/png').split(',')[1];
      }
    }
    if (!m && cropB64) return { pref: null, body: null, preprocessed, cropB64 };
    return m ? { ...m, preprocessed, cropB64 } : null;
...
async function measureFile(file, mime, wbox, pad = 0.03) {
  ...
  if (!prefix || !body) return { pref: null, body: null, preprocessed: true, cropB64: prep };

  const mm = await measurePage.evaluate(async ({ b64, prefix, body, MEASURE_FN }) => {
    eval(MEASURE_FN);
    const img = new Image();
  ...
  return mm ? { ...mm, cropB64: prep } : { pref: null, body: null, preprocessed: true, cropB64: prep };
}
...
for (const c of cases) {
  ...
  if (c.kind === 'gen') {
    ...
    const boxes = await page.evaluate(() => {
      ...
      return {
        prefix: r(document.querySelector('#pfx')),
        body: r(document.querySelector('#bodyref')),
        warn: r(document.querySelector('.warning')),
      };
    });
    ...
    m = await measureGen(file, mime, {
      ...
      warn: { x: boxes.warn.x * s, y: boxes.warn.y * s, w: boxes.warn.w * s, h: boxes.warn.h * s },
    });
  } else {
    file = c.file;
  ...
  // Full-label AI from prior rounds (comparison baseline); crop AI is the
  // experimental lens — a fresh call on the pre-processed warning crop.
  const fullAi = aiCache.get(c.name) ?? 'unclear';
  let cropAi = 'unclear';
  if (m?.cropB64) {
    fs.writeFileSync(path.join(OUT, `${c.name}.crop.png`), Buffer.from(m.cropB64, 'base64'));
    cropAi = await aiJudgment(`${c.name}-crop`, m.cropB64, 'png');
  }

  if (!m || !m.pref || !m.body) {
    results.push({ name: c.name, group: c.group, split: c.split, truthBold: c.truthBold, note: c.note, ai: cropAi, fullAi, measurable: false });
    console.log(`${c.name} [${c.group}/${c.split}]: PIXEL-UNMEASURABLE cropAi=${cropAi} fullAi=${fullAi}`);
    continue;
  }
  const r = {
    name: c.name, group: c.group, split: c.split, truthBold: c.truthBold, note: c.note, ai: cropAi, fullAi, measurable: true,
  ...
  results.push(r);
  console.log(`${c.name} [${c.group}/${c.split}]: sw=${r.swRatio} dens=${r.densRatio} size=${r.sizeRatio} cropAi=${cropAi} fullAi=${fullAi} truth=${c.truthBold ? 'BOLD' : 'not'}`);
}
...
// Standalone comparison: crop-fed AI vs full-label AI on all scored cases.
const scoredAll = results.filter((r) => !['semibold-600', 'all-bold'].includes(r.group));
for (const [label, key] of [['CROP-fed AI', 'ai'], ['full-label AI', 'fullAi']]) {
  const right = scoredAll.filter((r) => r[key] !== 'unclear' && (r[key] === 'bold') === r.truthBold).length;
  const wrong = scoredAll.filter((r) => r[key] !== 'unclear' && (r[key] === 'bold') !== r.truthBold).length;
  console.log(`${label}: ${right} right, ${wrong} wrong, ${scoredAll.length - right - wrong} unclear (n=${scoredAll.length})`);
}

fs.writeFileSync(path.join(OUT, 'croplens-results.json'), JSON.stringify({ results, gate: best }, null, 2));
console.log(`\nRaw results in ${OUT}/croplens-results.json`);
