...
6. **The government warning has its own panel**, split into *Wording* (word-for-word against 27 CFR 16.21) and *Formatting* (ALL CAPS, and bold). Bold is the one check a computer cannot finish: where the stroke-width measurement is confident it resolves itself and says so, and where it is not, the Formatting row grows the same **Accept** / **Reject** pair. (How accurate that is, measured: [docs/approach.md § How accurate is the bold check?](docs/approach.md#how-accurate-is-the-bold-check) — 1.9% worst case against 29% for a model alone.) Rejecting bold is a warning failure, and the whole screen turns red to match.
...
2. Results stream in as each label finishes. **Filter chips** carry live counts (All / Matched / Need review / Checking bold / Not required) and problems sort to the top when the run ends. *Need review* holds only rows where something was actually found; a clean row whose bold type is still being measured sits under the temporary grey **Checking bold** chip and moves out on its own as the measurement lands. **Search** filters by file name or brand. Keyboard: `↑` `↓` to move, `Enter` to open, `Esc` to close.
...
5. **Confirm bold** appears by itself when labels still need a human glance, showing only those rows as cropped warning images — hover one to magnify it, then **Accept** (looks bold) or **Reject** (not bold), the same two words as every other decision on the screen; every decision offers an **Undo**. The measurement pass runs for every batch — whatever it can settle it settles silently, and only the rows it couldn't reach the strip.
6. **Download report (CSV)** exports every row with both the machine verdicts and your rulings (`agent_review`: ok / correction; `bold_check`: agent_verified / agent_flagged / auto_verified / auto_flagged / unconfirmed — the prefix says who decided). Nothing is stored server-side — the exported report is the durable record, which is why **+ New batch** asks before discarding results you haven't downloaded.
...
| `single-check-upload.mjs` | The single-check page driven as a **user upload** — a label that is neither a demo card nor in the sample batch, chosen through the real file picker. Asserts the warning row, the audit trail's placement and that it names the uploaded file | stdout (exit 1 on failure) |
